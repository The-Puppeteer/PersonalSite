...
function Cone (n) {
   ...
   // var norm2 = vec4(0,-1,0,0);
    ...
    for(var i = 0; i < n; i++){
        ...
        pushVertices(vec4(0,0,0,1), vert_colors[2], vec4(0,-1,0,0)); //make the first disk,center vertex is flat
        pushVertices(vec4(Math.cos(ang),0, Math.sin(ang),1), vert_colors[4], vec4(0,-1,0,0));
        pushVertices(vec4(Math.cos(ang2),0, Math.sin(ang2),1),vert_colors[5], vec4(0,-1,0,0));
        
        pushVertices(vec4(0,1,0,1),vert_colors[2], vec4(Math.cos(ang),Math.sin(ang),0,0)); //make the next disk, stretch center vertex in the y direction
        pushVertices(vec4(Math.cos(ang),0,Math.sin(ang),1), vert_colors[4],vec4(Math.cos(ang),Math.sin(ang),0,0) );
        pushVertices(vec4(Math.cos(ang2),0,Math.sin(ang2),1),vert_colors[5], vec4(Math.cos(ang2),Math.sin(ang2),0,0));
