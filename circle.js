...
window.onload = function init(){ //begins during the loading of the page
    ...
    canvas.addEventListener("click", function(event) { // adds a click event listener for the canvas
    	var circle = new Circle(event.x - rect.left, event.y - rect.top, Math.random()*30, "black"); //creates new local variable circle and sets x & y from mouse pos, r to a random int*30 and the color to black
